...
/* !!! FIXME: eventually remove support for Xinerama and XVidMode (everything below here). */
       ...
       This block should be removed with the XVidMode support. */
    {
        if (SDL_GetHintBoolean("SDL_VIDEO_X11_REQUIRE_XRANDR", SDL_FALSE)) {
            ...
            #else
            return SDL_SetError("XRandR support is required but not built into SDL2!");
            #endif
        }
    }
...
#if SDL_VIDEO_DRIVER_X11_XINERAMA
        /* Most of SDL2's calls to X11 are unwaware of Xinerama, and to X11 standard calls, when Xinerama is active,
...
X11_GetDisplayModes(_THIS, SDL_VideoDisplay * sdl_display)
{
     ...
     * visual and depth ahead of time, but the SDL2 API allows you to create
