...
/* GLX implementation of SDL2 OpenGL support */
...
#else
        return SDL_SetError("SDL2 not configured with EGL support");
#endif
    }
...
X11_GL_GetAttributes(_THIS, Display * display, int screen, int * attribs, int size, Bool for_FBConfig, int **_pvistypeattr)
{
    ...
    /* assert buffer is large enough to hold all SDL2 attributes. */
...
X11_GL_CreateContext(_THIS, SDL_Window * window)
{
    ...
    if (vinfo) {
        if (_this->gl_config.major_version < 3 &&
            ...
            /* SDL2 profile bits match GLX profile bits */
            if( _this->gl_config.profile_mask != 0 ) {
                attribs[iattr++] = GLX_CONTEXT_PROFILE_MASK_ARB;
                attribs[iattr++] = _this->gl_config.profile_mask;
            }

            /* SDL2 flags match GLX flags */
