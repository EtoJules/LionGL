...
# error SDL2 for Mac OS X must be built with a 10.7 SDK or above.
...
/* Handle drag-and-drop of files onto the SDL2 window. */
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
     ...
     * window is resizable and not in a SDL2 fullscreen mode.
     */
...
- (void)flagsChanged:(NSEvent *)theEvent
{
    ...
    /* Catch capslock in here as a special case:
       ...
       to send duplicate key events; SDL2 filters them out quickly in
       SDL_SendKeyboardKey(). */
...
{ @autoreleasepool
{
    ...
    /* Fill in the SDL2 window with the window data */
    {
        NSRect rect = [nswindow contentRectForFrameRect:[nswindow frame]];
        ConvertNSRect([nswindow screen], (window->flags & FULLSCREEN_MASK), &rect);
