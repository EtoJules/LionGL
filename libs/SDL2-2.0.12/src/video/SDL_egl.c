...
int SDL_EGL_SetErrorEx(const char * message, const char * eglFunctionName, EGLint eglErrorCode)
{
    ...
    if (errorText[0] == '\0') {
        /* An unknown-to-SDL2 error code was reported.  Report its hexadecimal value, instead of its name. */
...
static SDL_bool SDL_EGL_HasExtension(_THIS, SDL_EGL_ExtensionType type, const char *ext)
{
     ...
     *  Bit   Action
     *  0     If set, the display extension is masked and not present to SDL2.
     *  1     If set, the client extension is masked and not present to SDL2.
     */
...
SDL_EGL_CreateContext(_THIS, EGLSurface egl_surface)
{
    ...
    if ((_this->gl_config.flags & SDL_GL_CONTEXT_DEBUG_FLAG) != 0) {
        ...
        if (((egl_version_major < 1) || (egl_version_major == 1 && egl_version_minor < 5)) &&
            ...
            /* SDL2 profile bits match EGL profile bits. */
    ...
    if ((major_version < 3 || (minor_version == 0 && profile_es)) &&
         ...
         * version is less than 3.0 (matches SDL2's GLX/WGL behavior.)
         */
        if (profile_es) {
    ...
    if (SDL_EGL_MakeCurrent(_this, egl_surface, egl_context) < 0) {
        /* Save the SDL2 error set by SDL_EGL_MakeCurrent */
        char errorText[1024];
