                              ...
                              WINRT_CursorNormalizationType normalization)
{
    ...
    // If the coordinates need to be sized to the SDL2 window,
    // we'll do that after.
...
WINRT_ProcessMouseMovedEvent(SDL_Window * window, Windows::Devices::Input::MouseEventArgs ^args)
{
    ...
    // position to the screen's center (when SDL2's relative mouse mode is enabled),
    ...
    // to SDL2 with minimal transformation: from native screen coordinates (in DIPs)
    // to SDL2 window coordinates.
    //
