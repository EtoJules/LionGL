...
/* WinRT SDL2 video driver implementation

   Initial work on this was done by David Ludwig (dludwig@pobox.com), and
   was based off of SDL2's "dummy" video driver.
 */
...
/* [re]declare Windows GUIDs locally, to limit the amount of external lib(s) SDL2 has to link to */
...
VideoBootStrap WINRT_bootstrap = {
    WINRTVID_DRIVER_NAME, "SDL2 WinRT video driver",
...
WINRT_AddDisplaysForOutput (_THIS, IDXGIAdapter1 * dxgiAdapter1, int outputIndex)
{
    ...
    if (hr == DXGI_ERROR_NOT_CURRENTLY_AVAILABLE) {
        /* DXGI_ERROR_NOT_CURRENTLY_AVAILABLE gets returned by IDXGIOutput::FindClosestMatchingMode
           ...
           In this case, just add an SDL2 display mode, with approximated values.
        */
        ...
        mode.refresh_rate = 0;  /* Display mode is unknown, so just fill in zero, as specified by SDL2's header files */
    ...
    for (int outputIndex = 0; ; ++outputIndex) {
        if (WINRT_AddDisplaysForOutput(_this, dxgiAdapter1, outputIndex) < 0) {
            /* HACK: The Windows App Certification Kit 10.0 can fail, when
               ...
               This was caused by SDL2/WinRT's DXGI failing to report any
               ...
               the Windows App Certification Kit, or possibly in SDL2/WinRT's
...
WINRT_UpdateWindowFlags(SDL_Window * window, Uint32 mask)
{
    ...
    if (window) {
        ...
        if ((apply & mask) & SDL_WINDOW_FULLSCREEN) {
            window->last_fullscreen_flags = window->flags;  // seems necessary to programmatically un-fullscreen, via SDL2 APIs
        }
...
WINRT_IsCoreWindowActive(CoreWindow ^ coreWindow)
{
    /* WinRT does not appear to offer API(s) to determine window-activation state,
       at least not that I am aware of in Win8 - Win10.  As such, SDL2 tracks this
    ...
    */
    ...
    /* Assume that non-SDL2 tracked windows are active, although this should
       probably be avoided, if possible.
       
       This might not even be possible, in normal SDL2 use, at least as of
       this writing (Dec 22, 2015; via latest hg.libsdl.org/SDL2 clone)  -- DavidL
    */
    return true;
}
...
WINRT_CreateWindow(_THIS, SDL_Window * window)
{
    ...
    /* To note, when XAML support is enabled, access to the CoreWindow will not
       be possible, at least not via the SDL2/XAML thread.  Attempts to access it
...
WINRT_GetWindowWMInfo(_THIS, SDL_Window * window, SDL_SysWMinfo * info)
{
    ...
    if (info->version.major <= SDL_MAJOR_VERSION) {
        ...
        SDL_SetError("Application not compiled with SDL2 %d.%d",
