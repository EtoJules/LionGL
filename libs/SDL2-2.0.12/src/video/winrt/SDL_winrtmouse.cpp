...
WINRT_ShowCursor(SDL_Cursor * cursor)
{
    ...
    if (cursor) {
        ...
        //      certification.
        //
        // SDL2 can use either means of hiding the cursor.  It provides a Win32-style
        // set of cursor resource files in its source distribution, inside
        // src/main/winrt/.  If those files are linked to an SDL2-for-WinRT/UWP app
        // (by including them in a MSVC project, for example), SDL2 will attempt to
        // use those, if and when the cursor is hidden via SDL2 APIs.  If those
        // files are not linked in, SDL2 will attempt to hide the cursor via the
