...
class SDL_BWin:public BDirectWindow
{
  public:
    ...
    virtual BGLView *CreateGLView(Uint32 gl_flags) {
        Lock();
        if (_SDL_GLView == NULL) {
            _SDL_GLView = new BGLView(Bounds(), "SDL2 GLView",
                                     B_FOLLOW_ALL_SIDES,
...
/* FIXME: Why were these here?
 ...
 if true:  SDL_SetCursor(NULL); */
    ...
    void _RepaintEvent() {
        /* Force a repaint: Call the SDL2 exposed event */
    ...
    /* Command methods (functions called upon by SDL2) */
