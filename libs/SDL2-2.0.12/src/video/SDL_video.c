...
SDL_VideoInit(const char *driver_name)
{
    ...
    /* Disable the screen saver by default. This is a change from <= 2.0.1,
       but most things using SDL2 are games or media players; you wouldn't
       want a screensaver to trigger if you're playing exclusively with a
       joystick, or passively watching a movie. Things that use SDL2 but
    ...
    /* If we don't use a screen keyboard, turn on text input by default,
       ...
       in SDL2 1.2 before you got text input events.  Hmm...
     */
...
SDL_SetDisplayModeForDisplay(SDL_VideoDisplay * display, const SDL_DisplayMode * mode)
{
    ...
    if (!_this->SetDisplayMode) {
        return SDL_SetError("SDL2 video driver doesn't support changing display mode");
    }
...
SDL_UpdateFullscreenMode(SDL_Window * window, SDL_bool fullscreen)
{
    ...
    /* HACK: WinRT 8.x apps can't choose whether or not they are fullscreen
       ...
       Just look at what SDL2's WinRT video backend code detected with regards
...
SDL_CreateWindow(const char *title, int x, int y, int w, int h, Uint32 flags)
{
    ...
    if (flags & SDL_WINDOW_OPENGL) {
        if (!_this->GL_CreateContext) {
            SDL_SetError("OpenGL support is either not configured in SDL2 "
                         "or not available in current SDL2 video driver "
    ...
    if (flags & SDL_WINDOW_VULKAN) {
        if (!_this->Vulkan_CreateSurface) {
            SDL_SetError("Vulkan support is either not configured in SDL2 "
                         "or not available in current SDL2 video driver "
                         "(%s) or platform", _this->name);
            return NULL;
        }
...
SDL_RecreateWindow(SDL_Window * window, Uint32 flags)
{
    ...
    if ((flags & SDL_WINDOW_OPENGL) && !_this->GL_CreateContext) {
        return SDL_SetError("OpenGL support is either not configured in SDL2 "
                            "or not available in current SDL2 video driver "
                            "(%s) or platform", _this->name);
    }
...
SDL_GL_LoadLibrary(const char *path)
{
    ...
    if (_this->gl_config.driver_loaded) {
        ...
        if (!_this->GL_LoadLibrary) {
            return SDL_SetError("No dynamic GL support in current SDL2 video driver (%s)", _this->name);
        }
...
SDL_GL_GetProcAddress(const char *proc)
{
    ...
    if (_this->GL_GetProcAddress) {
        ...
        SDL_SetError("No dynamic GL support in current SDL2 video driver (%s)", _this->name);
    }
    return func;
}
...
SDL_GL_SetAttribute(SDL_GLattr attr, int value)
{
    ...
    case SDL_GL_CONTEXT_EGL:
        /* FIXME: SDL_GL_CONTEXT_EGL to be deprecated in SDL2 2.1 */
        if (value != 0) {
...
SDL_GL_GetAttribute(SDL_GLattr attr, int *value)
{
    ...
    *value = 0;
    ...
    case SDL_GL_DOUBLEBUFFER:
        ...
        /* SDL2 driver must set proper value after initialization              */
...
SDL_GL_CreateContext(SDL_Window * window)
{
    ...
    /* Creating a context is assumed to make it current in the SDL2 driver. */
    if (ctx) {
...
int SDL_Vulkan_LoadLibrary(const char *path)
{
    ...
    if (_this->vulkan_config.loader_loaded) {
        ...
        if (!_this->Vulkan_LoadLibrary) {
            return SDL_SetError("Vulkan support is either not configured in SDL2 "
                                "or not available in current SDL2 video driver "
                                "(%s) or platform", _this->name);
        }
