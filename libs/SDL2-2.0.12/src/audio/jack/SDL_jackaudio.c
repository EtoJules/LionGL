...
jackShutdownCallback(void *arg)  /* JACK went away; device is lost. */
{
    ...
    SDL_SemPost(this->hidden->iosem);  /* unblock the SDL2 thread. */
}
...
jackProcessPlaybackCallback(jack_nframes_t nframes, void *arg)
{
    ...
    SDL_SemPost(this->hidden->iosem);  /* tell SDL2 thread we're done; refill the buffer. */
    return 0;  /* success */
}
...
jackProcessCaptureCallback(jack_nframes_t nframes, void *arg)
{
    ...
    SDL_SemPost(this->hidden->iosem);  /* tell SDL2 thread we're done; new buffer is ready! */
    return 0;  /* success */
}
...
JACK_OpenDevice(_THIS, void *handle, const char *devname, int iscapture)
{
    /* Note that JACK uses "output" for capture devices (they output audio
        ...
        Likewise, SDL2's playback port will be "output" (we write data out)
    ...
    client = JACK_jack_client_open("SDL2", JackNoStartServer, &status, NULL);
    ...
    /* Build SDL2's ports, which we will connect to the device ports. */
...
JACK_Init(SDL_AudioDriverImpl * impl)
{
    if (LoadJackLibrary() < 0) {
        ...
        jack_status_t status;
        jack_client_t *client = JACK_jack_client_open("SDL2", JackNoStartServer, &status, NULL);
        if (client == NULL) {
            UnloadJackLibrary();
            return 0;
        }
