...
/*
  The PulseAudio target for SDL2 1.3 is based on the 1.3 arts target, with
...
getAppName(void)
{
    ...
    return "SDL2 Application";  /* oh well. */
}
...
PULSEAUDIO_CaptureFromDevice(_THIS, void *buffer, int buflen)
{
    ...
    while (SDL_AtomicGet(&this->enabled)) {
        ...
        if (data == NULL) {  /* NULL==buffer had a hole. Ignore that. */
            ...
            /* store this fragment's data, start feeding it to SDL2. */
...
PULSEAUDIO_OpenDevice(_THIS, void *handle, const char *devname, int iscapture)
{
    ...
    /* The SDL2 ALSA output hints us that we use Windows' channel mapping */
