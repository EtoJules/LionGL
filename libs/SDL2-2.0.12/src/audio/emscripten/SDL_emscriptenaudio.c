...
EMSCRIPTENAUDIO_OpenDevice(_THIS, void *handle, const char *devname, int iscapture)
{
    ...
    if (iscapture) {
        /* The idea is to take the capture media stream, hook it up to an
           ...
           to access the raw PCM samples and push them to the SDL2 app's
        ...
        EM_ASM_({
            ...
            var have_microphone = function(stream) {
                //console.log('SDL2 audio capture: we have a microphone! Replacing silence callback.');
                if (SDL2.capture.silenceTimer !== undefined) {
                    clearTimeout(SDL2.capture.silenceTimer);
            ...
            var no_microphone = function(error) {
                //console.log('SDL2 audio capture: we DO NOT have a microphone! (' + error.name + ')...leaving silence callback running.');
            };
...
AudioBootStrap EMSCRIPTENAUDIO_bootstrap = {
    "emscripten", "SDL2 emscripten audio driver", EMSCRIPTENAUDIO_Init, 0
};
