...
/* Audio targets should call this as devices are removed, so SDL2 can update
...
/* Audio targets should call this if an opened audio device is lost while
   ...
   as appropriate so SDL2's list of devices is accurate. */
...
typedef struct SDL_AudioDriverImpl
{
    ...
    void (*FreeDeviceHandle) (void *handle);  /**< SDL2 is done with handle from SDL_AddAudioDevice() */
...
/* Define the SDL2 audio driver structure */
struct SDL_AudioDevice
{
    ...
    /* Scratch buffer used in the bridge between SDL2 and the user callback. */
    Uint8 *work_buffer;
