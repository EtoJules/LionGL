...
/*
 * XAML-to-SDL2 Rendering Callback
 */
#if WINAPI_FAMILY == WINAPI_FAMILY_APP
...
SDL_WinRTInitXAMLApp(int (*mainFunction)(int, char **), void * backgroundPanelAsIInspectable)
{
    ...
    // Make sure the app is ready to call the SDL2-centric main() function:
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) < 0) {
        // SDL_InitSubSystem will, on error, set the SDL2 error.  Let that propogate to
        // the caller to here:
