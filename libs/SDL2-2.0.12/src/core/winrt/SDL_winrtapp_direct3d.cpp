...
// SDL2/WinRT will use this throughout its code.
//
...
WINRT_SetDisplayOrientationsPreference(void *userdata, const char *name, const char *oldValue, const char *newValue)
{
    ...
    /* HACK: prevent SDL2 from altering an app's .appxmanifest-set orientation
...
void SDL_WinRTApp::SetWindow(CoreWindow^ window)
{
    ...
    // Register the hint, SDL_HINT_ORIENTATIONS, with SDL2.
...
void SDL_WinRTApp::OnVisibilityChanged(CoreWindow^ sender, VisibilityChangedEventArgs^ args)
{
    ...
    if (WINRT_GlobalSDLWindow) {
        ...
        // HACK: Prevent SDL2's window-hide handling code, which currently
        // triggers a fake window resize (possibly erronously), from
        // marking the SDL2 window's surface as invalid.
        //
    ...
    */
    ...
    if (window) {
        if (args->WindowActivationState != CoreWindowActivationState::Deactivated) {
            ...
            // after a user alt-tabs back into a full-screened SDL2 app.
...
void SDL_WinRTApp::OnSuspending(Platform::Object^ sender, SuspendingEventArgs^ args)
{
    ...
    create_task([this, deferral]()
    {
        ...
        // once it sends out a suspend event.  Any events posted to SDL2's
...
WINRT_LogPointerEvent(const char * header, Windows::UI::Core::PointerEventArgs ^ args, Windows::Foundation::Point transformedPoint)
{
    ...
    SDL_Log("%s: Position={%f,%f}, Transformed Pos={%f, %f}, MouseWheelDelta=%d, FrameId=%d, PointerId=%d, SDL2 button=%d\n",
        header,
