...
Android_JNI_SetEnv(JNIEnv *env) {
    ...
    if (status < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Failed pthread_setspecific() in Android_JNI_SetEnv() (err=%d)", status);
    }
    return status;
}
...
JNIEnv* Android_JNI_GetEnv(void)
{
    ...
    if (env == NULL) {
        ...
        if (mJavaVM == NULL) {
            __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Failed, there is no JavaVM");
            return NULL;
        }
        ...
        if (status < 0) {
            __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Failed to attach current thread (err=%d)", status);
            return NULL;
        }
...
Android_JNI_CreateKey(void)
{
    ...
    if (status < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Error initializing mThreadKey with pthread_key_create() (err=%d)", status);
    }
}
...
Android_JNI_CreateKey_once(void)
{
    ...
    if (status < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Error initializing mThreadKey with pthread_once() (err=%d)", status);
    }
}
...
register_methods(JNIEnv *env, const char *classname, JNINativeMethod *methods, int nb)
{
    ...
    if (clazz == NULL || (*env)->RegisterNatives(env, clazz, methods, nb) < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Failed to register methods of %s", classname);
        return;
    }
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    if ((*mJavaVM)->GetEnv(mJavaVM, (void **)&env, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "Failed to get JNI Env");
...
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "nativeSetupJNI()");
    ...
    if (mJavaVM == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "failed to found a JavaVM");
    }
    ...
    if (Android_ActivityMutex == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "failed to create Android_ActivityMutex mutex");
    }


    Android_PauseSem = SDL_CreateSemaphore(0);
    if (Android_PauseSem == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "failed to create Android_PauseSem semaphore");
    }

    Android_ResumeSem = SDL_CreateSemaphore(0);
    if (Android_ResumeSem == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "failed to create Android_ResumeSem semaphore");
    }
    ...
    if (!midClipboardGetText ||
        ...
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "Missing some Java callbacks, do you have the latest version of SDLActivity.java?");
    }
...
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "AUDIO nativeSetupJNI()");
    ...
    if (!midAudioOpen || !midAudioWriteByteBuffer || !midAudioWriteShortBuffer || !midAudioWriteFloatBuffer || !midAudioClose ||
       !midCaptureOpen || !midCaptureReadByteBuffer || !midCaptureReadShortBuffer || !midCaptureReadFloatBuffer || !midCaptureClose || !midAudioSetThreadPriority) {
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "Missing some Java callbacks, do you have the latest version of SDLAudioManager.java?");
    }
...
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "CONTROLLER nativeSetupJNI()");
    ...
    if (!midPollInputDevices || !midPollHapticDevices || !midHapticRun || !midHapticStop) {
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "Missing some Java callbacks, do you have the latest version of SDLControllerManager.java?");
    }

    checkJNIReady();
}

/* SDL2 main function prototype */
typedef int (*SDL_main_func)(int argc, char *argv[]);

/* Start up the SDL2 app */
JNIEXPORT int JNICALL SDL_JAVA_INTERFACE(nativeRunMain)(JNIEnv *env, jclass cls, jstring library, jstring function, jobject array)
{
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "nativeRunMain()");
    ...
    if (library_handle) {
        ...
        if (SDL_main) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "SDL2", "nativeRunMain(): Couldn't find function %s in library %s", function_name, library_file);
        }
        ...
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "nativeRunMain(): Couldn't load library %s", library_file);
    }
    ...
    /* Do not issue an exit or the whole application will terminate instead of just the SDL2 thread */
    /* exit(status); */
                                    ...
                                    JNIEnv *env, jclass cls)
{
    ...
    str = SDL_GetError();
    if (str && str[0]) {
        __android_log_print(ANDROID_LOG_ERROR, "SDL2", "SDLActivity thread ends (error=%s)", str);
    } else {
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "SDLActivity thread ends");
    }
}
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativePause)(
                                    JNIEnv *env, jclass cls)
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "nativePause()");
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeResume)(
                                    JNIEnv *env, jclass cls)
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "nativeResume()");

    /* Signal the resume semaphore so the event loop knows to resume and restore the GL Context
     * We can't restore the GL Context here because it needs to be done on the SDL2 main thread
                                    ...
                                    JNIEnv *env, jclass cls, jboolean hasFocus)
{
    ...
    if (Android_Window) {
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "nativeFocusChanged()");
...
/*******************************************************************************
             Functions called by SDL2 into Java
*******************************************************************************/
...
int Android_JNI_OpenAudioDevice(int iscapture, SDL_AudioSpec *spec)
{
    ...
    if (iscapture) {
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "SDL2 audio: opening device for capture");
        result = (*env)->CallStaticObjectMethod(env, mAudioManagerClass, midCaptureOpen, spec->freq, audioformat, spec->channels, spec->samples);
    } else {
        __android_log_print(ANDROID_LOG_VERBOSE, "SDL2", "SDL2 audio: opening device for output");
    ...
    if (jbufobj == NULL) {
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "SDL2 audio: could not allocate an audio buffer");
...
void Android_JNI_WriteAudioBuffer(void)
{
    ...
    case ENCODING_PCM_FLOAT:
        ...
        break;
    default:
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "SDL2 audio: unhandled audio buffer format");
        break;
    }
...
int Android_JNI_CaptureAudioBuffer(void *buffer, int buflen)
{
    ...
    case ENCODING_PCM_FLOAT:
        ...
        break;
    default:
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "SDL2 audio: unhandled capture buffer format");
        break;
    }
    return br;
}
...
void Android_JNI_FlushCapturedAudio(void)
{
    ...
    case ENCODING_PCM_FLOAT:
        ...
        break;
    default:
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "SDL2 audio: flushing unhandled capture buffer format");
        break;
    }
#else
    switch (captureBufferFormat) {
...
fallback:
        ...
        /* __android_log_print(ANDROID_LOG_DEBUG, "SDL2", "Falling back to legacy InputStream method for opening file"); */
        ...
        if (Android_JNI_ExceptionOccurred(SDL_FALSE)) {
             ...
             * If no exception occurred then the last SDL2 error message is kept.
             */
...
//////////////////////////////////////////////////////////////////////////////
//
// Functions exposed to SDL2 applications in SDL_system.h
//////////////////////////////////////////////////////////////////////////////
*/
...
int SDL_AndroidGetExternalStorageState(void)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "SDL2", "external storage state: %s", state);
...
void Android_JNI_GetManifestEnvironmentVariables(void)
{
    if (!mActivityClass || !midGetManifestEnvironmentVariables) {
        __android_log_print(ANDROID_LOG_WARN, "SDL2", "Request to get environment variables before JNI is ready");
        return;
    }
