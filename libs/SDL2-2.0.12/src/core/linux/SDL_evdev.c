    ...
    if (keycode < SDL_arraysize(linux_scancode_table)) {
        ...
        if (scancode == SDL_SCANCODE_UNKNOWN) {
            /* BTN_TOUCH is handled elsewhere, but we might still end up here if
               you get an unexpected BTN_TOUCH from something SDL2 believes is not
               a touch device. In this case, we'd rather not get a misleading
               SDL_Log message about an unknown key. */
            if (keycode != BTN_TOUCH) {
                SDL_Log("The key you just pressed is not recognized by SDL2. To help "
                    "get this fixed, please report this to the SDL2 forums/mailing list "
...
SDL_EVDEV_sync_device(SDL_evdevlist_item *item) 
{
    ...
    *mt_req_code = ABS_MT_TRACKING_ID;
    ...
    for(i = 0; i < item->touchscreen_data->max_slots; i++) {
         ...
         * have to tell SDL2 of the two events, but since we wait till SYN_REPORT in
         * SDL_EVDEV_Poll to tell SDL2, the current structure of this code doesn't
