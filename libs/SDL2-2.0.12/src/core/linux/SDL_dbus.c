...
/* Don't do this - bug 3950
   dbus_shutdown() is a debug feature which closes all global res in the dbus library. Calling this should be done by the app, not a library, because if there are multiple users of dbus in the process then SDL2 could shut it down even though another part is using it.
*/
#if 0
    if (dbus.shutdown) {
        dbus.shutdown();
