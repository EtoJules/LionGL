...
*
* Fix for SDL2 bug https://bugzilla.libsdl.org/show_bug.cgi?id=3437:
* The extra 'SDL_' was added to the start of each IID's name, in order
* to prevent build errors on both MinGW-w64 and WinRT/UWP.
* (SDL2 bug https://bugzilla.libsdl.org/show_bug.cgi?id=3336 led to
...
D3D11_CreateDeviceResources(SDL_Renderer * renderer)
{
    ...
    if (!D3D11_CreateBlendState(renderer, SDL_BLENDMODE_BLEND) ||
        ...
        /* D3D11_CreateBlendMode will set the SDL2 error, if it fails */
        goto done;
    }
...
D3D11_HandleDeviceLost(SDL_Renderer * renderer)
{
    ...
    if (FAILED(result)) {
        /* D3D11_CreateDeviceResources will set the SDL2 error */
        return result;
    }

    result = D3D11_UpdateForWindowSizeChange(renderer);
    if (FAILED(result)) {
        /* D3D11_UpdateForWindowSizeChange will set the SDL2 error */
        return result;
    }
...
D3D11_CreateTexture(SDL_Renderer * renderer, SDL_Texture * texture)
{
    ...
    if (textureFormat == DXGI_FORMAT_UNKNOWN) {
        return SDL_SetError("%s, An unsupported SDL2 pixel format (0x%x) was specified",
...
D3D11_UpdateViewport(SDL_Renderer * renderer)
{
    ...
    /* Make sure the SDL2 viewport gets rotated to that of the physical display's rotation.
    ...
    if (rendererData->cliprectDirty) {
        if (!rendererData->currentCliprectEnabled) {
            ...
            if (D3D11_GetViewportAlignedD3DRect(renderer, &rendererData->currentCliprect, &scissorRect, TRUE) != 0) {
                /* D3D11_GetViewportAlignedD3DRect will have set the SDL2 error */
                return -1;
            }
