...
AllocateRenderCommand(SDL_Renderer *renderer)
{
    ...
    /* !!! FIXME: are there threading limitations in SDL2's render API? If not, we need to mutex this. */
...
SDL_GetRenderDriverInfo(int index, SDL_RendererInfo * info)
{
    ...
    *info = render_drivers[index]->info;
    return 0;
#else
    return SDL_SetError("SDL2 not built with rendering support");
#endif
}
...
#else
    SDL_SetError("SDL2 not built with rendering support");
    return NULL;
#endif
}
...
#else
            SDL_SetError("SDL2 not built with YUV support");
#endif
            if (!texture->yuv) {
                SDL_DestroyTexture(texture);
                return NULL;
