...
/* This is the version of the dynamic API. This doesn't match the SDL2 version
...
/* BE CAREFUL CALLING ANY SDL2 CODE IN HERE, IT WILL BLOW UP.
...
initialize_jumptable(Uint32 apiver, void *table, Uint32 tablesize)
{
    ...
    if (apiver != SDL_DYNAPI_VERSION) {
        ...
        return -1;  /* newer version of SDL2 with functions we can't provide. */
    }
    ...
    /* Safe to call SDL2 functions now; jump table is initialized! */
...
{
    const char *caption = "SDL2 Dynamic API Failure!";
...
/* This is not declared in any header, although it is shared between some
    parts of SDL2, because we don't want anything calling it without an
...
SDL_InitDynamicAPILocked(void)
{
    ...
    if (libname) {
        ...
        if (!entry) {
            dynapi_warn("Couldn't load overriding SDL2 library. Please fix or remove the SDL_DYNAMIC_API environment variable. Using the default SDL2.");
    ...
    if (entry) {
        if (entry(SDL_DYNAPI_VERSION, &jump_table, sizeof (jump_table)) < 0) {
            dynapi_warn("Couldn't override SDL2 library. Using a newer SDL2 build might help. Please fix or remove the SDL_DYNAMIC_API environment variable. Using the default SDL2.");
            /* Just fill in the function pointers from this library, later. */
        } else {
            use_internal = SDL_FALSE;   /* We overrode SDL2! Don't use the internal version! */
        }
    }
    ...
    if (use_internal) {
        if (initialize_jumptable(SDL_DYNAPI_VERSION, &jump_table, sizeof (jump_table)) < 0) {
            ...
            dynapi_warn("Failed to initialize internal SDL2 dynapi. As this would otherwise crash, we have to abort now.");
            SDL_ExitProcess(86);
        }
    }
...
SDL_InitDynamicAPI(void)
{
     ...
     *  insanely rare, as you would have to spin a thread outside of SDL2 (as
     *  SDL_CreateThread() would also call this function before building the
     *  new thread).
     */
    static SDL_bool already_initialized = SDL_FALSE;

    /* SDL_AtomicLock calls SDL2 mutex functions to emulate if
